...
class ControllerExtensionModuleSolistingtabs extends Controller {
	public function index($setting) {
		...
		if($is_ajax && isset($_POST['is_ajax_listing_tabs']) && $_POST['is_ajax_listing_tabs']){
			...
			if (!file_exists(DIR_STORAGE.'modification/catalog/view/theme/'.$this->config->get('theme_default_directory') . '/template/extension/module/so_listing_tabs/'.$setting['store_layout'].'/default_items.twig')) {
				...
				$url_items_tdd = 'modification/catalog/view/theme/'.$this->config->get('theme_default_directory') . '/template/extension/module/so_listing_tabs/'.$setting['store_layout'].'/default_items.twig';	
				$loader = new Twig_Loader_Filesystem(DIR_STORAGE.'modification/catalog/view/theme/'.$this->config->get('theme_default_directory') . '/template/extension/module/so_listing_tabs/');
			}
			// $url_items_tdd = $this->config->get('theme_default_directory') . '/template/extension/module/so_listing_tabs/'.$setting['store_layout'].'default_items.twig';
			...
			$product_info->items_markup = preg_replace(
					array(
							'/ {2,}/',
							'/<!--.*?-->|\t|(?:\r?\n[ \t]*)+/s'
					),
	...
	public function getSubCategories($parent_id = 0) {
		...
		foreach ($query->rows as $product_info) {
			$product_info['url_link']	= $this->url->link('product/category', 'path='.$product_info['category_id']);
			$subcat[]	= $product_info;
		}
		return $subcat;
	}
		...
		if ($cat['count'] > 0) 
		{
			...
			foreach($products_arr as $product_info)
			{
				...
				$data['first_gallery'] = array(
						'cart' => $this->model_tool_image->resize($product_info['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_cart_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_cart_height')),
						'thumb' => $this->model_tool_image->resize($product_info['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_product_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_product_height'))
				);
	...
	private function processCategory($catids){
		...
		if (empty($catpubid)) {
			...
			$catids = $this->model_extension_module_so_listing_tabs->getCategories_son_listing_tabs($filter_data);
