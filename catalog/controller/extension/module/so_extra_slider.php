...
class ControllerExtensionModuleSoextraslider extends Controller {
			...
			foreach($products_arr as $product)
			{
				...
				if(version_compare(VERSION, '2.1.0.2', '>')) {
					...
					if ((float)$product_info['special']) {
						...
						$discount = '-'.round((($product_info['price'] - $product_info['special'])/$product_info['price'])*100, 0).'%';					} else {
						$special = false;
						$discount = false;					}
	...
	private function processCategory($catids)
	{
		...
		if (empty($catpubid)) {
			...
			$catids = $this->model_extension_module_so_extra_slider->getCategories_son($filter_data);
